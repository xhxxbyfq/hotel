...
public class Predetermine {
	...
	public ModelAndView tolist(HttpServletRequest request, Integer currentPage,
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (shiJianCha>=0) {
				...
				this.predetermineService.updateRemind(huoQuShiJian,huoQuId);
			}
		}
	...
	public ModelAndView delete(String id){
		...
		for (int i = 0; i < FenGe.length; i++) {
			...
			PredeterminePo predeterminePo = predetermineService.findById(Integer.parseInt(FenGe[i]));
			RoomSetPo roomSetPo = roomSetService.selectById(predeterminePo.getRoomID());
	...
	public ModelAndView arrangeRoom(String id,Integer tiaoZhuang){
		...
		for (int i = 0; i < FenGe.length; i++) {
            ...
            this.predetermineService.updatePredetermineStateID(predeterminePo.getArriveTime(),Integer.parseInt(FenGe[i]));              //修改预订状态
            ...
            stayRegisterPo.setPredetermineArriveTime(predeterminePo.getArriveTime());
